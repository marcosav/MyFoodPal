...
public class PreferenceManager {
    ...
    private static final String MFP_COOKIES = "mfp_cookies";
    ...
    public Cookies saveCookies(String cookies) {
        preferences.edit().putString(MFP_COOKIES, cookies).apply();
    ...
    public Cookies getCookies() {
        var got = preferences.getString(MFP_COOKIES, null);
        if (got != null)
            return deserializeCookies(got);
        return null;
    }

    private Cookies deserializeCookies(String serializedCookies) {
        var cookies = new HashMap<String, String>();
        var tokenizedCookies = serializedCookies.split("; ");

        Stream.of(tokenizedCookies).forEach(c -> {
            var parts = c.split("=", 2);
